...
add_action('admin_print_scripts', array('Resume_Field', 'admin_hook_scripts'));
add_action('admin_print_styles', array('Resume_Field', 'admin_hook_styles'));

if ( !class_exists('Resume_Field') ) :
...
class Resume_Field {
	...
	static function factory($type, $name, $label=null) {
		...
		$class = 'Resume_Field_' . $type;

		if (!class_exists($class)) {
			throw new Resume_Exception ('Unknown field "' . $type . '".');
		}

		if ( strpos($name, '-') !== false ) {
			throw new Resume_Exception ('Forbidden character "-" in name "' . $name . '".');
		}
	 ...
	 * @return object $this
	 **/
	function set_datastore(Resume_DataStore $store) {
		$this->store = $store;
		return $this;
	}
	...
	function set_label($label) {
		...
		if (is_null($label)) {
			...
			// remove the leading "rbf_"(if it's there)
			$label = preg_replace('~^rbf_~', '', $label);
	 ...
	 * The class is stripped by the "Resume_Field" prefix. Then underscores are replaced with a dash.
	...
	protected function clean_type($type) {
		$clean_class = str_replace('Resume_Field', '', $type);
	...
	protected function parse_conditional_rules($rules) {
		if ( ! is_array( $rules ) ) {
			throw new Resume_Exception('Conditional logic rules argument should be an array.');
		}
		...
		foreach ( $rules as $key => $rule ) {
			...
			if ( ! is_array($rule) || empty( $rule['field'] ) ) {
				throw new Resume_Exception('Invalid conditional logic rule format. The rule should be an array with the "field" key set.');
			}
			...
			if ( ! in_array( $rule['compare'], $allowed_operators ) ) {
				throw new Resume_Exception('Invalid conditional logic compare oparator: <code>' . $rule['compare'] . '</code><br>' . 
					'Allowed oparators are: <code>' . implode(', ', $allowed_operators) . '</code>');
			}
			if ( $rule['compare'] === 'IN' || $rule['compare'] === 'NOT IN' ) {
				if ( ! is_array( $rule['value'] ) ) {
					throw new Resume_Exception('Invalid conditional logic value format. An array is expected, when using the "' . $rule['compare'] . '" operator.');
				}
			}
	...
	static function admin_hook_scripts() {
		wp_enqueue_media();
		wp_enqueue_script('resume-fields', RESUME_PLUGIN_URL . '/js/fields.js', array('resume-app', 'resume-containers'));
		wp_localize_script('resume-fields', 'rbfl10n',
			array(
				'title' => __('Files', 'rbf'),
				'geocode_zero_results' => __('The address could not be found. ', 'rbf'),
				'geocode_not_successful' => __('Geocode was not successful for the following reason: ', 'rbf'),
				'max_num_items_reached' => __('Maximum number of items reached (%s items)', 'rbf'),
				'max_num_rows_reached' => __('Maximum number of rows reached (%s rows)', 'rbf'),
				'cannot_create_more_rows' => __('Cannot create more than %s rows', 'rbf'),
				'enter_name_of_new_sidebar' => __('Please enter the name of the new sidebar:', 'rbf'),
				'remove_sidebar_confirmation' => __('Are you sure you wish to remove this sidebar?', 'rbf'),
				'add_sidebar' => __('Add Sidebar', 'rbf'),
				'complex_no_rows' => __('There are no %s yet. Click <a href="#">here</a> to add one.', 'rbf'),
				'complex_add_button' => __('Add %s', 'rbf'),
				'complex_min_num_rows_not_reached' => __('Minimum number of rows not reached (%d %s)', 'rbf'),

				'message_form_validation_failed' => __('Please fill out all fields correctly. ', 'rbf'),
				'message_required_field' => __("This field is required. ", 'rbf'),
				'message_choose_option' => __("Please choose an option. ", 'rbf'),
			)
		);
	}
...
if ( !class_exists('Resume_Field_Text') ) :

class Resume_Field_Text extends Resume_Field {
	function template() {
		?>
...
endif; // END Resume_Field_Text


if ( !class_exists('Resume_Field_Textarea') ) :

class Resume_Field_Textarea extends Resume_Field {
...
endif; // END Resume_Field_Textarea


if ( !class_exists('Resume_Field_Rich_Text') ) :

class Resume_Field_Rich_Text extends Resume_Field_Textarea {
	...
	function template() {
		?>
		<div id="wp-{{{ id }}}-wrap" class="resume-wysiwyg wp-editor-wrap tmce-active" data-toolbar="full">
	...
	function editor_init() {
		...
		<div style="display:none;">
			<?php
				...
				wp_editor('', 'resume_settings', $settings);
...
if ( !class_exists('Resume_Field_Date') ) :

class Resume_Field_Date extends Resume_Field {
	...
	function template() {
		...
		<div class="input-with-button">
			<input id="{{{ id }}}" type="text" name="{{{ name }}}" value="{{ value }}" class="regular-text resume-datepicker" />
			<span class="resume-datepicker-trigger button icon-button hide-if-no-js"><?php _e('Date', 'rbf'); ?></span>
		</div>
		<?php
	}
	...
	function admin_enqueue_scripts() {
		...
		wp_enqueue_style('resume-jquery-ui', RESUME_PLUGIN_URL . '/css/jquery-ui.css');
	}
...
if ( !class_exists('Resume_Field_Color') ) :

class Resume_Field_Color extends Resume_Field {
	function template() {
		?>
		<div class="resume-color-row">
			<div class="input-with-button">
				<input id="{{{ id }}}" type="text" name="{{{ name }}}" value="{{ value }}" class="regular-text resume-color" />
				<span class="pickcolor button icon-button hide-if-no-js"><?php _e('Select a Color', 'rbf'); ?></span>
			</div>
			<div class="resume-color-container hide-if-no-js"></div>
		</div>
		<?php
	}
...
if ( !class_exists('Resume_Field_Map') ) :

class Resume_Field_Map extends Resume_Field {
	protected $lazyload = true;
	...
	function admin_enqueue_scripts() {
		wp_enqueue_script('resume-google-maps', '//maps.googleapis.com/maps/api/js?sensor=false');
	}
	...
	function template() {
		?>
		<div class="resume-map-search">
			<p><?php _e('Locate Address on the map', 'rbf'); ?>: </p>
		...
		<div class="resume-map-canvas">&nbsp;</div>
		<?php
	}
...
if ( !class_exists('Resume_Field_Map_With_Address') ) :

class Resume_Field_Map_With_Address extends Resume_Field_Map {
	function get_type() {
...
if ( !class_exists('Resume_Field_Select') ) :

class Resume_Field_Select extends Resume_Field {
	function set_options($options) {
...
if ( !class_exists('Resume_Field_Radio') ) :

class Resume_Field_Radio extends Resume_Field_Select {
	function template() {
		?>
		<# if (_.isEmpty(options)) { #>
			<em><?php _e('no options', 'rbf'); ?></em>
		<# } else { #>
			<ul class="resume-radio-list">
				<# _.each(options, function(option) { #>
					<li>
						<label>
...
if ( !class_exists('Resume_Field_Checkbox') ) :

class Resume_Field_Checkbox extends Resume_Field {
...
if ( !class_exists('Resume_Field_Header_Scripts') ) :

class Resume_Field_Header_Scripts extends Resume_Field_Textarea {
	function init() {
		$this->help_text(__('If you need to add scripts to your header, you should enter them here.', 'rbf'));
	...
	function print_scripts() {
		if ( !$this->store || !is_a($this->store, 'Resume_DataStore_ThemeOptions') ) {
			return;
		}
...
if ( !class_exists('Resume_Field_Footer_Scripts') ) :

class Resume_Field_Footer_Scripts extends Resume_Field_Textarea {
	function init() {
		$this->help_text(__('If you need to add scripts to your footer (like Google Analytics tracking code), you should enter them in this box.', 'rbf'));
...
if ( !class_exists('Resume_Field_Separator') ) :

class Resume_Field_Separator extends Resume_Field {
	function template() {
		?>
		<h3>{{{ label }}}</h3>
		<?php
...
if ( !class_exists('Resume_Field_Set') ) :

class Resume_Field_Set extends Resume_Field {
			...
			<div class="resume-set-list">
				<# _.each(options, function(option, i) { #>
					...
					<# if (!exceed && !last && counter == limit_options) { #>
						<p>... <a href="#" class="resume-set-showall"><?php _e('Show All Options', 'rbf'); ?></a></p>
					<# } #>
				<# }) #>
			</div>
		<# } #>
...
if ( !class_exists('Resume_Field_Relationship') ) :

class Resume_Field_Relationship extends Resume_Field {
	 ...
	 *
	 * Can be overriden or extended by the `resume_relationship_title` filter.
	 * 
	...
	function get_title_by_type($id, $type, $subtype = '') {
		 ...
		 */
		return apply_filters('resume_relationship_title', $title, $this->get_name(), $id, $type, $subtype);
	}

	/**
	 * Used to get the label of an item. 
	 *
	 * Can be overriden or extended by the `resume_relationship_item_label` filter.
	 * 
	 * @param int     $id      The database ID of the item.
	 * @param string  $type    Item type (post, term, user, comment, or a custom one).
	 * @param string  $subtype Subtype - "page", "post", "category", etc.
	...
	function get_item_label($id, $type, $subtype = '') {
		 ...
		 * @param string $subtype Subtype - "page", "post", "category", etc.
		 */
		return apply_filters('resume_relationship_item_label', $label, $this->get_name(), $id, $type, $subtype);
	}
	...
	function get_options() {
		...
		foreach ($this->post_type as $post_type) {
			$filter_name = 'resume_relationship_options_' . $this->get_name() . '_post_' . $post_type;
		 ...
		 * @param string $name Name of the relationship field.
		 */
		$options = apply_filters('resume_relationship_options', $options, $this->get_name());
		...
		<div class="relationship-container">
			<div class="relationship-left">
				<div class="search-field">
					<input type="text" class="search-field" placeholder="<?php esc_attr_e('Search', 'rbf'); ?>" />
				</div>
			...
			<div class="relationship-right">
				<label><?php _e('Associated:', 'rbf'); ?></label>
...
if ( !class_exists('Resume_Field_Association') ) :

class Resume_Field_Association extends Resume_Field_Relationship {
	...
	*/
	...
	function get_title_by_type($id, $type, $subtype = '') {
		...
		if ($type === 'post') {
			...
			$max = apply_filters('resume_relationship_comment_length', 30, $this->get_name());
			if (strlen($title) > $max) {
	...
	function get_options() {
		...
		foreach ($this->types as $type) {
			...
			if ($type['type'] === 'post') {
				 ...
				 * @param array $args The parameters, passed to get_posts().
				 */
				$filter_name = 'resume_relationship_options_' . $this->get_name() . '_' . $type['type'] . '_' . $type['post_type'];
				$args = apply_filters($filter_name, array(
			...
			// populate taxonomy terms
				 ...
				 */
				$filter_name = 'resume_relationship_options_' . $this->get_name() . '_' . $type['type'] . '_' . $type['taxonomy'];
				$args = apply_filters($filter_name, array(
					'hide_empty' => 0,
					'fields' => 'id=>name',
				));
			...
			// populate users
				 ...
				 */
				$filter_name = 'resume_relationship_options_' . $this->get_name() . '_' . $type['type'];
				$args = apply_filters($filter_name, array(
					'fields' => 'ID'
				));
	...
	function to_json($load) {
		$field_data = Resume_Field::to_json($load);
...
if ( !class_exists('Resume_Field_File') ) :

class Resume_Field_File extends Resume_Field {
	public $button_label = '';
	...
	function admin_init() {
		$this->button_label = __('Select File', 'rbf');
		$this->window_button_label = __('Select File', 'rbf');
		$this->window_label = __('Files', 'rbf');
	...
	function template() {
		...
		<div class="input-with-button">
			<input 
				...
				name="{{ name }}" 
				value="{{ value }}" 
				class="regular-text resume-file-field" 
	...
	function template_description() {
		?>
		<div class="resume-description {{{ value ? '' : 'hidden' }}}">
			<a href="{{ value }}" target="_blank" class="button resume-view_file">
				<?php _e('View File', 'rbf'); ?>
			</a>
		</div>
		<?php
	}
...
if ( !class_exists('Resume_Field_Attachment') ) :

class Resume_Field_Attachment extends Resume_Field_File {
	public $value_type = 'id';

	function template_description() {
		?>
		<div class="resume-description {{{ value ? '' : 'hidden' }}}">
			<div class="resume-attachment-preview {{{ thumb_url ? '' : 'hidden' }}}">
				<div class="resume-preview">
					...
					<div class="resume-file-remove"></div>
				</div>
			</div>
...
if ( !class_exists('Resume_Field_Image') ) :

class Resume_Field_Image extends Resume_Field_Attachment {
	public $field_type = 'image';
	public $value_type = 'url';
	
	function admin_init() {
		$this->button_label = __('Select Image', 'rbf');
		$this->window_button_label = __('Select Image', 'rbf');
		$this->window_label = __('Images', 'rbf');
...
if ( !class_exists('Resume_Field_Sidebar') ) :

class Resume_Field_Sidebar extends Resume_Field_Select {
	...
	function to_json($load) {
		if ($this->enable_add_new) {
			$this->options['new'] = _x('Add New', 'sidebar', 'rbf');
		}
...
if ( !class_exists('Resume_Field_Choose_Sidebar') ) :

class Resume_Field_Choose_Sidebar extends Resume_Field_Sidebar {
	...
	function init() {
		...
		add_action('resume_after_register_fields', array($this, 'setup_sidebar_options'), 20);
		add_action('resume_after_register_fields', array($this, 'register_custom_sidebars'), 21);
	}
	...
	function get_default_sidebar_options() {
		...
		if ( function_exists( 'rbf_get_default_sidebar_options' ) ) {
			$sidebar_options = rbf_get_default_sidebar_options();
		}
...
if ( !class_exists('Resume_Field_HTML') ) :

class Resume_Field_HTML extends Resume_Field {
	public $field_html;
...
if ( !class_exists('Resume_Field_Gravity_Form') ) :

/**
 * Gravity Form Select
 */
class Resume_Field_Gravity_Form extends Resume_Field_Select {
	function admin_init() {
		// Setup Form Options
		add_action( 'resume_after_register_fields', array($this, 'setup_gravity_form_options'), 20 );
	}
	...
	function to_json($load) {
		...
		$this->set_options( apply_filters( 'rbf_gravity_form_options', $this->options ) );
	...
	function template() {
		...
		if ( !$this->is_plugin_active() ) {
			?><em><?php _e('Please install Gravity Forms plugin', 'rbf'); ?></em><?php
			return;
		}

		// No forms have been found
		if ( empty($this->options) ) {
			?><em><?php _e('No Gravity Forms have been found.', 'rbf'); ?></em><?php
			return;
		}
