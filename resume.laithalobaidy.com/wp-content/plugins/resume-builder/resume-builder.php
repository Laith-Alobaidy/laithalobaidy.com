...
define('RB_FA_VERSION','4.4.0');

# Load Framework
if ( !class_exists('Resume_Field') ) {
	require_once dirname(__FILE__) . '/resume-fields/resume-fields.php';
}
...
function resume_builder_admin_init() {
	/* Load the custom Resume Fields styling */
   	wp_enqueue_style( 'resume-custom', plugins_url('css/resume-custom.css', __FILE__) );
}
...
function resume_builder_init() {
	...
	wp_enqueue_style( 'rb-font-awesome', '//maxcdn.bootstrapcdn.com/font-awesome/'.RB_FA_VERSION.'/css/font-awesome.min.css', array(), RB_FA_VERSION );
 ...
 */
add_action('resume_register_fields', 'rb_register_resume_meta_boxes');
function rb_register_resume_meta_boxes() {
	Resume_Container::factory('custom_fields', __('Resume Builder'))
		->show_on_post_type('rb_resume')
		->add_fields(array(
			# Resume Shortcode
			Resume_Field::factory('separator', 'rb_resume_shortcode_title', __('Resume Shortcodes', 'resume-builder')),
			Resume_Field::factory('html', 'rb_resume_shortcode')
				->set_html( rb_get_resume_shortcode_help_text() ),

			# Complex Sections
			Resume_Field::factory('separator', 'rb_resume_sections_title', __('The Resume Builder', 'resume-builder')),
			Resume_Field::factory('complex', 'rb_resume_sections', __('Resume Sections', 'resume-builder'))
				->setup_labels(array(
				...
				->add_fields('introduction_block', array(
					# Section Title
					Resume_Field::factory('text', 'sectiontitle', __('Introduction Title', 'resume-builder')),

					# Section SubTitle
					Resume_Field::factory('text', 'sectionsubtitle', __('Introduction Subtitle', 'resume-builder')),

					# Image
					Resume_Field::factory('attachment', 'sectionimage', __('Introduction Image', 'resume-builder'))
						->help_text(
							__(
					...
					# Text
					Resume_Field::factory('rich_text', 'sectiontext', __('Introduction Text', 'resume-builder')),

					# Buttons
					Resume_Field::factory('complex', 'sectionbuttons', __('Introduction Buttons', 'resume-builder'))
						->setup_labels(array(
						...
						->add_fields(array(
							# Label
							Resume_Field::factory('text', 'label', __('Button Label', 'resume-builder'))
								->set_required(true),

							# Link
							Resume_Field::factory('text', 'link', __('Button Link', 'resume-builder'))
								->set_required(true),

							# Type
							Resume_Field::factory('select', 'type', __('Button Style', 'resume-builder'))
								->set_required(true)
								->add_options(array(
				...
				->add_fields('default_block', array(
					# Section Title
					Resume_Field::factory('text', 'sectiontitle', __('Section Title', 'resume-builder')),

					# Section Content
					Resume_Field::factory('complex', 'sectioncontent', __('Section Content', 'resume-builder'))
						->setup_labels(array(
						...
						->add_fields('text_block', array(
							# Text
							Resume_Field::factory('rich_text', 'text', __('Text', 'resume-builder'))
								->set_required(true),
						))
						->add_fields('detailed_row', array(
							# Row Title
							Resume_Field::factory('text', 'rowtitle', __('Row Title', 'resume-builder')),

							# Row Subtitle
							Resume_Field::factory('text', 'rowsubtitle', __('Row Subtitle', 'resume-builder')),

							# Row Side Text
							Resume_Field::factory('text', 'rowsidetext', __('Row Side Text', 'resume-builder')),

							# Row Text
							Resume_Field::factory('rich_text', 'rowtext', __('Row Text', 'resume-builder')),
						)),
				)),

			# Widgets
			Resume_Field::factory('separator', 'rb_resume_widget', __('Resume Widgets', 'resume-builder')),

				# Widget Contacts Title
				Resume_Field::factory('text', 'rb_resume_widget_contacts_title', __('Contact Info Title', 'resume-builder')),

					# Email
					Resume_Field::factory('text', 'rb_resume_widget_contacts_email', __('Email', 'resume-builder')),

					# Phone
					Resume_Field::factory('text', 'rb_resume_widget_contacts_phone', __('Phone', 'resume-builder')),

					# Website
					Resume_Field::factory('text', 'rb_resume_widget_contacts_website', __('Website', 'resume-builder')),

					# Address
					Resume_Field::factory('text', 'rb_resume_widget_contacts_address', __('Address', 'resume-builder')),

				# Widget Title
				Resume_Field::factory('text', 'rb_resume_widget_skills_title', __('Skills Title', 'resume-builder')),

					# Complex Skills
					Resume_Field::factory('complex', 'rb_resume_widget_skills', __('Skills', 'resume-builder'))
						->setup_labels(array(
						...
						->add_fields(array(
							# Title
							Resume_Field::factory('text', 'title', __('Title', 'resume-builder')),

							# Rating
							Resume_Field::factory('select', 'rating', __('Star Rating', 'resume-builder'))
								->add_options( rb_generate_rating() ),

							# Text
							Resume_Field::factory('textarea', 'text', __('Description', 'resume-builder')),
						)),
		));
}
...
function rb_get_resume_shortcode_help_text() {
	...
	if ( is_admin() && isset($_GET['post']) ) {
		...
		if (!is_array($post_ID)){
			...
			$output  = '<strong style="width:200px; display:inline-block;">'.__('Full Resume: ', 'resume-builder') . '</strong><code>[rb_resume id="' . $post_ID . '"]</code><br/>';
			$output .= '<strong style="width:200px; display:inline-block;">'.__('Resume Intro: ', 'resume-builder') . '</strong><code>[rb_resume_intro id="' . $post_ID . '"]</code><br/>';
			$output .= '<strong style="width:200px; display:inline-block;">'.__('Resume Widget Skills: ', 'resume-builder') . '</strong><code>[rb_resume_widget_skills id="' . $post_ID . '"]</code><br/>';
			$output .= '<strong style="width:200px; display:inline-block;">'.__('Resume Widget Contacts: ', 'resume-builder') . '</strong><code>[rb_resume_widget_contacts id="' . $post_ID . '"]</code><br/>';
...
function rb_resume_widget_skills_shortcode($atts, $content = null) {
	...
	# Widget Title
	$widget_title = resume_get_post_meta($resume->ID, 'rb_resume_widget_skills_title');
	if ( !empty($widget_title) ) {
		echo '<h3 class="widget-title">' . $widget_title . '</h3>';
	}

	# Widget Skills
	$skills = resume_get_post_meta($resume->ID, 'rb_resume_widget_skills', 'complex');
	if ($skills) {
...
function rb_resume_widget_contacts_shortcode($atts, $content = null) {
	...
	$title   = resume_get_post_meta($resume->ID, 'rb_resume_widget_contacts_title');
	$email   = resume_get_post_meta($resume->ID, 'rb_resume_widget_contacts_email');
	$phone   = resume_get_post_meta($resume->ID, 'rb_resume_widget_contacts_phone');
	$website = resume_get_post_meta($resume->ID, 'rb_resume_widget_contacts_website');
	$address = resume_get_post_meta($resume->ID, 'rb_resume_widget_contacts_address');
